...
if use_corr:
    ...
    if api_key_corr:
        ...
        # Mostrar tabela com conteúdos corrigidos
...
if analise == "Clusterização Ementas":
    ...
    # Nomear clusters via ChatGPT, com diagnóstico de erro
        ...
        if api_key_gpt:
            ...
            for cid in range(k):
                ...
                prompt = (
                    ...
                    "Em 1 a 3 palavras, sem aspas, dê um único nome que resuma o tema comum:\n\n"
                    f"{exemplos_str}"
                )
                try:
            ...
            st.sidebar.warning("Informe a API Key para habilitar nomeação.")
            # Fallback: centróide
            for cid in range(k):
...
# --------------------------------------------------
    ...
    with pd.ExcelWriter(buf, engine='xlsxwriter') as writer:
        ...
        start = xl_rowcol_to_cell(1,1); end = xl_rowcol_to_cell(nrows,ncols)
    ...
    st.download_button("⬇️ Baixar Matriz de Similaridade", buf,
                       "sim_enade_ementa_colorido.xlsx",
                       "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
...
# --------------------------------------------------
    ...
    df_group2 = (df_ementas.groupby('COD_EMENTA')['CONTEUDO_PROGRAMATICO']
...
# --------------------------------------------------
    ...
    st.download_button("⬇️ Baixar Análise vs ENADE", buf,
                       "analise_ementa_vs_enade.xlsx",
                       "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    st.subheader("Frequência de UCs ≥ limiar")
    col_uc = f"UCs_>={int(limiar*100)}%"
    lista_cod = df_res[col_uc].str.split(r';\s*').explode().dropna().astype(str)
    ...
    buf_fig.seek(0)
    st.download_button("⬇️ Baixar gráfico de frequência", buf_fig,
                       "frequencia_ementas.png","image/png")
